...
export class ForoComponent implements OnInit  {
  ...
  crearPublicacion() {
    if (this.publicaciones && this.publicaciones.length > 0) {
      ...
      const lastId = Math.max(...this.publicaciones.map((pub: Publicacion) => parseInt(pub.id || '0'))) || 0;
      this.publicacion.id = (lastId + 1).toString();
    } else {
      // Si no hay publicaciones, el id inicial es 1
